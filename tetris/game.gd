	...
	for x in range(tetrimino.shape.size()):
		for y in range(tetrimino.shape[x].size()):
			if tetrimino.shape[x][y] == 1:
				...
				if new_x < 0 or new_x >= tetris_grid.GRID_WIDTH or new_y >= tetris_grid.GRID_HEIGHT:
				...
				if movement.y > 0:
					...
					if y == tetrimino.shape[x].size() - 1:  # Bottom row
						...
						if tetris_grid.grid[new_x][new_y] != 0:
	...
	for x in range(tetrimino.shape.size()):
		for y in range(tetrimino.shape[x].size()):
			if tetrimino.shape[x][y] == 1:
				...
				tetris_grid.grid[grid_x][grid_y] = 1  # Mark the grid position as occupied
